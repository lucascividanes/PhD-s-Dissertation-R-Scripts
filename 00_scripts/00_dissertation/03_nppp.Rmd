...
nppp_npos <- read_delim("G:/Meu Drive/Acadêmicos/Doutorado/PhD-s-Dissertation-R-Scripts/02_out/nppp_npos.csv", 
  ...
  mutate(PCT_NPPP = round(N_NPPP/sum(N_NPPP),3)) %>% 
  arrange(-N_NPPP) %>% 
...
```{r network, fig.cap = "The Secretary of Culture NPPPs financed via IBAs network (2019). Source: author. Data: São Paulo City Hall.", warning = FALSE, echo = FALSE, fig.show='hold', out.width="75%", fig.align='center', fig.width = 8, fig.height = 6}
...
ggplot(ggnetwork(net, layout = "fruchtermanreingold"), aes(x, y, xend = xend, yend = yend)) +
  ...
  geom_nodes(aes(color = class, shape = type), size = 7) +
  geom_nodelabel(aes(label = vertex.names), color = "black", fontface = "bold", size = 1) +
  theme_blank() +
...
aux_out_degree <- read_excel("G:/Meu Drive/Acadêmicos/Doutorado/PhD-s-Dissertation-R-Scripts/01_in/iba/sao_paulo/2019_emendas_R01.xlsx", 
  ...
  summarise(N = n(),
            ...
            mean_out_degree = round(mean(out_degree, na.rm = T), 2))
...
aux_in_degree <- nppp_edges %>% 
  ...
  group_by(CLASS) %>% 
  summarise(mean_in_degree = round(mean(in_degree, na.rm = T), 2))
...
Results so far are only exploratory. A more comprehensive dataset based on NPPPs for the last two legislatures and concerning all secretaries will provide a better picture of the phenomena under study. Adjustments should be made to the classification as well, since the distribution now seems very unbalanced. Moreover, further descriptive metrics and inferential analysis are intended to be used to test the hypotheses drawn before.
